...
/** @class */
function () {
  function Bullet(id, position, range, speed, damage) {
    var _this = this;
...
},{}],"assets/hitmark.mp3":[function(require,module,exports) {
...
function () {
  function FireArm() {
    ...
    this.fire = function (socket, position, target, time, lastFired, fireRate) {
      ...
      if (_this.mag === 0) {
        ...
        if ((time - lastFired) / 1000 >= 1 / fireRate) {
          socket.emit("fire", {
            ...
            target: target,
            userId: socket.id,
            id: socket.id + time + "" + position.x + position.y
          });
...
},{"../assets/empty.mp3":"assets/empty.mp3","../assets/reload.mp3":"assets/reload.mp3"}],"src/weapons/assaultRifle.ts":[function(require,module,exports) {
"use strict";
...
/** @class */
function () {
  function Character(playingScene, socket) {
    var _this = this;

    this.weapon = new assaultRifle_1.default();
    this.health = 100;
    this.moving = false;
    this.hit = new Audio(hitmark_mp3_1.default);
    this.healthBar = null;
    this.lastHealth = 100;
    this.aim = {
      x: 0,
    ...
    this.updatePosition = function (x, y, r) {
      _this.lastPosition = _this.position.x + _this.position.y;
      _this.position.x = x;
      _this.position.y = y;
      _this.radius;
    };
    ...
    //   if (
    //     this.position.x + this.speed * this.direction.x > 30 &&
    //     this.position.x + this.speed * this.direction.x < width - 20
    //   )
    //     this.position.x = this.position.x + this.speed * this.direction.x;
    //   if (
    //     this.position.y + this.direction.y * this.speed > 30 &&
    //     this.position.y + this.direction.y * this.speed < height - 40
    //   )
    //     this.position.y += this.direction.y * this.speed;
    // };


    this.updateHealth = function (hp) {
      _this.health = hp;
    };

    this.update = function () {
      if (_this.health < _this.lastHealth) {
        _this.hit.play();

        _this.lastHealth = _this.health;
      } // this.updatePosition(player.position.x, player.position.y);


      _this.socket.emit("playerUpdate", _this.direction);

      if (_this.lastPosition != _this.position.x + _this.position.y) _this.moving = true;else _this.moving = false; //updates the health bar
      ...
      if (_this.firing) {
        ...
        _this.weapon.fire(_this.socket, _this.position, {
          x: _this.aim.x,
          y: _this.aim.y
        }, _this.time, _this.weapon.lastFired, _this.weapon.fireRate);
      } // this.jumpLogic(width, height, yPos);
      // this.moveLogic(x);


      if (_this.moving) {
        if (_this.frameCounter % 8 === 0) _this.currentFrame = (_this.currentFrame + 1) % 5 + 8;
      } else {
        if (_this.frameCounter % 20 === 0) _this.currentFrame = (_this.currentFrame + 5) % 8;
      }
...
//   }
// };
},{"./GameContext":"src/GameContext.ts","/assets/FinnSprite.png":"assets/FinnSprite.png","/assets/hitmark.mp3":"assets/hitmark.mp3","./HP":"src/HP.ts","./weapons/assaultRifle":"src/weapons/assaultRifle.ts","./Inventory":"src/Inventory.ts"}],"assets/bubble.wav":[function(require,module,exports) {
...
},{"./Scene":"src/Scene.ts","./GameContext":"src/GameContext.ts","./PlayingScene":"src/PlayingScene.ts","./PrettyMainMenuScene":"src/PrettyMainMenuScene.ts","/assets/FinnSprite.png":"assets/FinnSprite.png"}],"src/PlayingScene.ts":[function(require,module,exports) {
"use strict";
...
function (_super) {
  ...
  function PlayingScene(engine) {
    ...
    _this.syncServerState = function () {
      _this.socket.on("state", function (_a) {
        var players = _a.players,
            projectiles = _a.projectiles;
        _this.serverState.players = players;
        _this.serverState.projectiles = projectiles;
      });
    };

    _this.deleteGhosts = function () {
      for (var id in _this.characters) {
        if (!(id in _this.serverState.players)) {
          delete _this.characters[id];
        }
      }
    };

    _this.renderBullets = function () {
      for (var id in _this.serverState.projectiles) {
        var _a = _this.serverState.projectiles[id].position,
            x = _a.x,
            y = _a.y;
        var curr = _this.serverState.projectiles[id];

        if (id in _this.bullets) {
          _this.bullets[id].updatePosition(x, y);
        } else {
          _this.bullets[id] = new Bullet_1.default(id, curr.position, 10, 10, 10);
        }

        _this.bullets[id].render();
      }
    };

    _this.renderCharacters = function () {
      for (var id in _this.serverState.players) {
        var _a = _this.serverState.players[id].position,
            x = _a.x,
            y = _a.y;

        if (id in _this.characters) {
          _this.characters[id].updatePosition(x, y);
        } else {
          _this.characters[id] = new Character_1.default(_this, _this.socket);

          _this.characters[id].updatePosition(x, y);
        }

        _this.characters[id].render();
      }
    };

    _this.updateCharacters = function () {
      for (var id in _this.serverState.players) {
        if (id in _this.characters) {
          _this.characters[id].updateHealth(_this.serverState.players[id].health);

          _this.characters[id].update();
        }
      }
    }; //done
    ...
    _this.render = function () {
      ...
      context.fillStyle = "#98c695";
      var _b = _this.characters[_this.socket.id].position,
          x = _b.x,
          y = _b.y;

      _this.renderCharacters();
      ...
      context.fillStyle = "#98c695";
      context.fillText("#" + _this.round, 145, 50); // context.translate(x - width / 2, y - height / 2);
      //update time
    ...
    _this.update = function () {
      ...
      _this.characters[_this.socket.id].update();

      _this.deleteGhosts();

      _this.syncServerState();

      _this.time = new Date().getTime(); // this.overlay.update();

      var _a = GameContext_1.default.context.canvas,
          width = _a.width,
          height = _a.height; // this.character.update();
      //render damage hits
      // for (let i = 0; i < this.damage.length; i++) {
    ...
    _this.keyUpHandler = function (event) {
      ...
      _this.characters[_this.socket.id].keyupHandler(key);
    };

    _this.mouseMoveHandler = function (event) {
      _this.characters[_this.socket.id].mouseMoveHandler(event);
    };
    ...
    _this.keyDownHandler = function (event, engine) {
      ...
      _this.characters[_this.socket.id].keydownHandler(key);
    };

    _this.socket = engine.socket;
    _this.characters = {};
    _this.characters[_this.socket.id] = new Character_1.default(_this, _this.socket);
    _this.serverState.players = _this.characters;
    _this.bullets = {};
    return _this;
  }
...
},{"./Scene":"src/Scene.ts","./Bullet":"src/Bullet.ts","./Character":"src/Character.ts","./PauseScene":"src/PauseScene.ts","./GameContext":"src/GameContext.ts","./WinningScene":"src/WinningScene.ts","/assets/hitmark.mp3":"assets/hitmark.mp3"}],"src/GoodbyeScene.ts":[function(require,module,exports) {
"use strict";
...
},{"./Scene":"src/Scene.ts","./GameContext":"src/GameContext.ts","./PlayingScene":"src/PlayingScene.ts","./PrettyMainMenuScene":"src/PrettyMainMenuScene.ts","/assets/FinnSprite.png":"assets/FinnSprite.png"}],"src/PrettyMainMenuScene.ts":[function(require,module,exports) {
"use strict";
...
},{"./GameContext":"src/GameContext.ts","./Time":"src/Time.ts","./PrettyMainMenuScene":"src/PrettyMainMenuScene.ts"}],"node_modules/parseuri/index.js":[function(require,module,exports) {
/**
 * Parses an URI
 *
...
},{"./url":"node_modules/socket.io-client/lib/url.js","socket.io-parser":"node_modules/socket.io-parser/index.js","./manager":"node_modules/socket.io-client/lib/manager.js","debug":"node_modules/debug/src/browser.js","./socket":"node_modules/socket.io-client/lib/socket.js"}],"src/index.ts":[function(require,module,exports) {
"use strict";
...
var socket = socket_io_client_1.default("http://localhost:5000"); // 178.128.190.193
...
var engine = new Engine_1.default(socket);
engine.start();
...
},{"./Engine":"src/Engine.ts","./GameContext":"src/GameContext.ts","socket.io-client":"node_modules/socket.io-client/lib/index.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
